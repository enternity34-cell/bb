...
const shUseMultiFileAuthState = async (folder) => {
  ...
  /**
   * Migrasi & hapus file JSON lama (supaya folder bersih)
   */
  const migrateAndCleanOldFiles = () => {
    ...
    for (const file of files) {
      ...
      if (file === "creds.json" || file.startsWith("session") || file.includes("-")) {
        try {
          ...
          if (match) {
            ...
            if (parsed) writeData(category, id, parsed);
          }
    ...
    if (migrated.length > 0) {
      console.log(`[AuthState] Migrated & cleaned ${migrated.length} old JSON files.`);
    }
  };

  migrateAndCleanOldFiles();
